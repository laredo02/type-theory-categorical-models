...
The most straight forward way to test this, is to check whether there exists some term that reduces to itself, that is \( M \curly^n M \) with $ n > 0 $. It turns out \( \Omega \equiv (\la x . \, x \, x) (\la x . \, x \, x) \) fullfills this requirement. Let us inspect what goes on as we keep $\beta$-reducing this term:
\[
...
Recursion is important since for instance Turing Machines may run programs that do not terminate. \magenta{elaborate...}
...
\begin{align*}
  \underline{\mathrm{gcd}}(a,b) &= 
                      ...
                      \end{cases}
\end{align*}
Here, when $b \neq 0$ the name gcd is used to refer to the same definition again. In the \lcalc \ such self-referencing definitions are not permited since the language does not allow names. \magenta{See [DefBNF]}. To bypass this setback the notion of fixed point is of particular interest.
\begin{definition}
...
\begin{example} Some simple functions that illustrate what fixed points are about:
  ...
  \]
\end{example}
This harmless concept satisfies an interesting property, namely that $ x = f(x) = f(f(x)) = f^n(x) $ effectively creating a chain every element $ f^{n+1}(x) = f(f^{n}(x))$, thus attaining self-reference without naming. This idempotent behaviour under application is important, since it showcases the ability of fixed points to induce recursion. Our quest now, is to find a term that can simulate this behaviour. To do this, let us translate the definition for fixed point into the $\lambda$-world.
\magenta{maybe use \circ in the chain since it might have some meaning in the context of categories},
\begin{definition}
  Given a lambda term \( L \in \La \), a term \( M \in \La \) is called a fixed point of \( L \) if \( M =_\beta L M \).
\end{definition}
\begin{theorem}
  For every \( L \in \La \), there exists \( M \in \La \) such that $ M $ is a fixed point for $ M $ i.e. $ M =_\beta L M. $\orange{ This theorem is usually referred to as the Fixed Point Theorem in the literature}.
\end{theorem}
...
\end{proof}
\begin{note}
  See how $ L M $ is the application of $ M $ to $ L $ and that using \magenta{\ref{} [RightApplication]} we can keep $\beta$-reducing $ M $ the same way we did for $ x = f(x) = f(f(x))\dots $ only this time using lambda terms:
  \[
    M \curly L \, M \curly L \, L \, M \curly^* L^n M.
  \]
\end{note}
Now that nameless self-reference has been proven to exist within the realm of the \lcalc, we take a step forward and introduce a few of the most important fixed point combinators and explore their nature.
