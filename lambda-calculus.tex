...
\end{definition}
\begin{example}
  \magenta{Beta reduction of a sample multiplication}
  \magenta{Notion behind ISZERO}
\end{example}
\begin{remark}
...
\end{note}
\begin{remark}
  Even though the $Y$ combinator is of theoretical interest since it allows us to find recursion within the \lcalc, it is of little practical use since it runs into non-termination e.g.
  \[
    \begin{aligned}
...
If a $\lambda$-term $t$ reduces to two normal forms $n_1$ and $n_2$, then they are alpha-equivalent:
\[
